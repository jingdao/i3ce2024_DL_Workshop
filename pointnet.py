...
class PointNet_Classification(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = torch.max(x3, 2, keepdim=False)[0]
        x5 = F.relu(self.bn4(self.fc1(x4)))
        x6 = F.relu(self.bn5(self.dropout(self.fc2(x5))))
        x7 = self.fc3(x6)
        x7 = F.log_softmax(x7, dim=-1)
        return x7
...
if __name__ == '__main__':
    model = PointNet_Classification(10)
    xyz = torch.rand(10, 3, 2048)
    print(model)
    print(model(xyz).shape)

    model = PointNet_Segmentation(10)
    xyz = torch.rand(10, 9, 2048)
    print(model)
    print(model(xyz).shape)
